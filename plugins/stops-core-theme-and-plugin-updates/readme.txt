...
* 9.0.11 : Change the order (priority) of auto_update_plugin/theme filters to make the 'Safe Mode' feature (Premium) work properly. Improve the way errors are handled before and after a plugin activation takes place (deactivated plugins must be reactivated immediately regardless of whether or not the plugins contain a fatal error). Change use of '$' to local/function scope, to prevent conflicts; a recommended update for all.

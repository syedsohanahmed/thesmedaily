...
function wpr5_article_post($keyword,$num,$start,$template) {
	...
	$search_url = "http://www.sooperarticles.com/search/?t=contents&s=$keyword2&p=$page";
	...
	$paras = $xpath->query("//h2[@class='entry-title']/a");
	...
	for ($i = $numb;  $i < $end; $i++ ) {
		...
		if(empty($para)) {
			...
			$paras1 = $xpath1->query("//div[@class='article']//h1");
			$para1 = $paras1->item(0);
			...
			$paras2 = $xpath2->query("//div[@class='abody']/div[@class='micro-share']"); 
			$para2 = $paras2->item(0);		
			...
			$paras3 = $xpath3->query("//footer/div[@class='author-signature']");		//$para = $paras->item(0);		
